...
이미지 여러 장을 조합별로 판독해 비교합니다. 사용법은 문서 모델 비교와 같고, **[기본으로 적용]한 조합이 판독 · 통합 화면에 그대로 쓰입니다** (두 화면에서는 고르지 않고 표시만).

- **조합 설정**
  - **CNN 모델** (이진 판단): 파인튜닝 모델 / 사전학습 모델. 파인튜닝 모델은 테스트 데이터(Kaggle 소아)로 미세조정한 이진 분류, 사전학습 모델은 TorchXRayVision 의 경화(Consolidation) 출력을 씁니다. 처리할 이미지와 성격이 같은 데이터로 실험해서 고릅니다.
  - **VLM 판독 초안 작성**과 **VLM 모델**
  - **VLM에 CNN 결과 보여 주기**: 끄면 두 판단을 독립적으로 받아 교차 검증합니다 (기본값 끔, 실험 결과로 정함).
- **VLM 민감도·특이도**: 판독에 성공한 이미지만으로 계산합니다. 판독 실패가 많아 대상이 적으면 `(n=5)` 처럼 표본 수를 붙이고, 10장 미만이면 흐리게 표시합니다.
- **점수**: 정답이 있으면 `0.5×CNN 이진 판단 균형 정확도 + 0.4×VLM 이진 판단 균형 정확도×VLM 판독 성공률 + 0.1×속도` 입니다. 균형 정확도는 (민감도 + 특이도) ÷ 2 입니다. 정답은 테스트 데이터의 파일 이름(Kaggle 소아)이나 동반 문서(IU 성인)에서 찾습니다.
- **이미지별 결과**: 정답과 같으면 ✓, 틀리면 ✗ 로 표시합니다. 칸을 누르면 그 작업의 판독 화면으로 이동합니다.

## CNN + VLM 판독 ➔ 판독 (`/image/process`)과 작업 상세 (`/image/jobs/{id}`)

![CNN + VLM 판독 결과 — Grad-CAM, 이진 판단, 다중 항목](images/xray-job.png)

- 이미지 한 장을 올려 판독합니다. **적용 중인 파이프라인**(CNN 모델, VLM 모델 · 판독 방식, 근거가 된 실험)이 표시되며, 바꾸려면 모델 비교에서 다른 조합을 기본으로 적용합니다.
- **이미지 · Grad-CAM**: 원본 위에 이진 판단과 다중 항목의 히트맵을 겹쳐 봅니다.
- **CNN 이진 판단**과 **다중 항목 18종**: 확률을 막대로, 판정 기준을 세로선으로 보여 줍니다. 기준을 넘은 항목이 강조됩니다. 기준은 항목마다 다릅니다 (테스트 데이터로 보정, `src/CnnService/config.toml`). 기준값 바로 위(+0.02 이내)는 주황색 **경계**로 표시합니다.
- **VLM 판독 초안**: 항목, 결론, 양성 의심 여부입니다.
- **교차 검증**: CNN과 VLM의 판단을 비교합니다. 이진 판단이 다르면 **사람 확인 필요**로 표시됩니다.

## CNN + VLM + 텍스트 추출 (`/multimodal`)

이미지와 그 이미지에 딸린 텍스트 문서를 함께 넣어 **이미지 분석과 문서가 맞는지** 확인하고 종합 보고서를 만듭니다.

1. 이미지와 **문서 이미지**를 올립니다. 문서는 OCR ➔ 읽기 순서 텍스트 ➔ LLM 요약으로 처리됩니다. 이미지 분석은 판독 화면과 같은 **적용 중인 파이프라인**을 쓰고, VLM 판독 · 문서 요약 · 종합 보고서에 쓰는 LLM 도 함께 표시됩니다 (화면에서 고르지 않음).
2. 결과 화면
   - **판정 배너**: 사람 확인이 필요한지와 불일치 수입니다.
   - **종합 보고서**: 문서 요약, CNN, VLM 결과만으로 작성하며 새 항목을 추가하지 않습니다. 어긋난 항목은 단정하지 않고 확인 필요로 적습니다.
   - **항목별 일치 비교**: 비교 항목 5개(테스트 데이터 기준: 폐렴, 심장비대, 흉수, 무기폐, 폐부종)를 문서 / CNN / VLM 별로 나란히 봅니다. 이진 판단 항목이 다르면 오류, 나머지는 참고로 표시합니다. CNN 확률이 기준값 바로 위(+0.02 이내)면 **경계**로 표시하고 비교하지 않습니다 (이 구간 양성은 대부분 틀림). 종합 보고서에도 경계 항목은 양성으로 넘기지 않습니다.
   - **문서 요약**: 작성 사유, 핵심 내용, 항목, 결론입니다. 원본 이미지와 OCR 텍스트도 볼 수 있습니다. **참고한 용어 정의** 에는 요약할 때 붙인 용어집 항목이 나옵니다.
   - **이미지 분석 상세**: CNN + VLM 판독 화면과 같습니다.
- 텍스트를 직접 넣는 방식(`reportText`)과 CNN 모델 · LLM 덮어쓰기는 평가 스크립트(`eval/multimodal_eval.py`)용 API 에만 남아 있습니다.
