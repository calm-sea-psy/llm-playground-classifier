...
/** Step 3 진행 단계: 이미지 분석(CNN·VLM·교차 검증) ➔ 문서(OCR·요약) ➔ 일치 비교 ➔ 종합 보고서 */
export const multimodalProgress: ProgressModel = {
  steps: ['업로드', '이미지 분석', '문서 요약', '일치 비교', '종합 보고서', '완료'],
  progressOf(job) {
    ...
    switch (job.status) {
      ...
      case 'CnnRunning':
        return { step: 1, percent: message.includes('다중 항목') ? 18 : 12 }
      case 'OcrRunning':
        return { step: 2, percent: 50 }
      case 'LlmRunning':
        if (message.includes('종합 보고서')) return { step: 4, percent: 85 }
        if (message.includes('문서 요약')) return { step: 2, percent: 58 }
