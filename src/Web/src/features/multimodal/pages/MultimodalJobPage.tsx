...
/** 작업 1건: 진행(SignalR) ➔ 종합 보고서 · 일치 비교 · 문서 요약 ➔ 이미지 분석 상세(Step 2 화면 재사용) */
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.key} className={r.agree === false ? 'disagree' : undefined}>
              ...
              <td title={r.cnnBorderline ? '기준값 +0.02 이내: 대부분 틀려 판단 보류 (문서와 비교 · 보고서 핵심 항목에서 뺌)' : undefined}>
  ...
  return (
    <>
      <section className={`card verdict ${result.needsReview ? 'verdict-bad' : 'verdict-ok'}`}>
        <strong>{result.needsReview ? '사람 확인 필요' : '이미지 분석과 문서가 크게 어긋나지 않습니다'}</strong>
        <span className="small">
          문서 vs CNN 불일치 {disagree}개 · 문서 {result.reportSource === 'ocr' ? '이미지(OCR)' : '텍스트'} · LLM{' '}
      ...
      <section className="card panel">
        <header className="panel-head">
          <h3>종합 보고서</h3>
          <span className="muted small">문서 요약 + CNN + VLM 판독만으로 작성 (새 항목 추가 안 함)</span>
        </header>
        {!final ? (
      ...
      <div className="image-result-grid">
        ...
        <section className="card panel">
          ...
          {showText ? (
            <>
              {result.reportSource === 'ocr' && <img className="report-image" src={reportFileUrl(jobId)} alt="문서 이미지" />}
              <pre className="report-text">{result.reportText}</pre>
            </>
          ) : !summary ? (
            <p className="text-bad small">문서 요약 형식 오류</p>
          ) : (
          ...
          {!showText && result.glossary != null && (
            <details className="glossary small">
              ...
              {result.glossary.length === 0 ? (
                <p className="muted">문서에 용어집의 용어가 없어 붙이지 않았습니다.</p>
              ) : (
                <ul>
              ...
              <p className="muted">문서에 나온 용어의 정의만 용어집에서 찾아 요약 프롬프트에 붙였습니다.</p>
            </details>
          )}
        </section>
      </div>

      <h3 className="section-title">이미지 분석 상세</h3>
