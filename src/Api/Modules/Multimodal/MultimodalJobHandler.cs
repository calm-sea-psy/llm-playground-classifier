    ...
    : IJobHandler
{
    public async Task<string> HandleAsync(Job job, JobReporter reporter, CancellationToken ct)
    {
        var settings = MultimodalSettings.FromJson(job.Settings)
            ?? throw new InvalidOperationException("통합 작업 설정(텍스트 문서 파일)이 없습니다");
        ...
        {
            await reporter.SetStatusAsync(job, JobStatus.OcrRunning, "문서 OCR 중", ct);
            OcrResult result;
        ...
        await reporter.SetStatusAsync(job, JobStatus.LlmRunning, $"문서 요약 중 ({model})", ct);
        ...
        if (summary.ParseError is { } se)
        {
            issues.Add(new("summary_schema", IssueSeverity.Error,
                summary.FirstError is null ? $"문서 요약 형식 오류 ({se})" : $"문서 요약 형식 오류, 재시도도 실패 ({se})"));
        }
        else if (summary.FirstError is { } fe)
        {
            issues.Add(new("summary_retry", IssueSeverity.Warning, $"문서 요약 첫 응답 형식 오류 ➔ 재시도로 성공 ({fe})"));
        }
        ...
        return $"종합 보고서 완료: {ImageAnalyzer.Describe(image)}, 문서 불일치 {disagree}개"
