...
같은 방법론을 **CNN + VLM 판독**(CNN과 VLM 교차 검증)과 **CNN + VLM + 텍스트 추출**로 확장해, 영상 AI와 LLM을 함께 쓰는 경우도 평가했습니다 (연구·평가용, 진단 불가).
...
**확장 사례: CNN + VLM + 텍스트 추출** — IU X-Ray 성인 120건 (정상 40, 이상 80)
...
| CNN + VLM 판독: 모델 비교 (조합 4개) | CNN + VLM 판독: 결과 (Grad-CAM · 18소견 · 교차 검증) |
|---|---|
| ![CNN + VLM 판독 실험 상세](docs/images/xray-experiment.png) | ![CNN + VLM 판독 결과](docs/images/xray-job.png) |
...
| [화면 사용법](docs/user-guide.md) | 모델 비교, 실험 상세, 문서 처리, CNN + VLM 판독, CNN + VLM + 텍스트 추출, 프롬프트, 서비스 상태 |
